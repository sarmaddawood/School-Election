  ...
  return (
    ...
    >
      <motion.div variants={itemVariants}>
        ...
        <p className="text-sm text-zinc-400">Nominate eligible students or teachers to positions</p>
      </motion.div>

      {elections.length === 0 || positions.length === 0 ? (
        <motion.div
          variants={itemVariants}
          className="glass-panel rounded-2xl p-8 text-center flex flex-col items-center justify-center space-y-3 shadow-xl border border-white/5"
        >
          <motion.div
          ...
          <p className="font-medium text-zinc-300">Setup Prerequisite Required</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full px-4 py-3 glass-input rounded-xl text-sm appearance-none cursor-pointer pr-10 text-white outline-none"
                  >
                    {elections.map((el) => (
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full px-4 py-3 glass-input rounded-xl text-sm appearance-none cursor-pointer pr-10 text-white disabled:opacity-50 outline-none"
                  >
                    {filteredPositions.map((pos) => (
                      <option key={pos.id} value={pos.id} className="bg-black text-white">
                        {pos.name}
                      </option>
                    ))}
                    {filteredPositions.length === 0 && (
                      <option value="" className="bg-black text-white">No positions configured</option>
                    )}
                  </select>
                  <ChevronDown
                    size={16}
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    {nonAdmins.length === 0 && (
                      <option value="" className="bg-black text-white">No eligible students/teachers found</option>
                    )}
                  </select>
                  <ChevronDown
                    size={16}
              ...
              <div className="space-y-1.5">
                <div className="flex justify-between items-center">
                  ...
                  <motion.button
                    ...
                    className="text-[10px] font-bold text-violet-400 hover:text-violet-300 disabled:text-zinc-500 transition-colors flex items-center gap-1 cursor-pointer bg-violet-500/10 hover:bg-violet-500/20 px-2 py-1 rounded-lg border border-violet-500/20 shadow-sm"
                  >
              ...
              <motion.button
                ...
                className="w-full py-3 bg-violet-600 hover:bg-violet-700 disabled:bg-violet-400/20 text-white rounded-xl font-semibold text-xs transition-all flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-violet-600/20"
              >
                <UserPlus size={14} />
          ...
          <motion.div
            variants={itemVariants}
            className="lg:col-span-7 glass-panel rounded-2xl p-5 md:p-6 shadow-2xl space-y-6 border border-white/5"
          >
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-white/5 pb-3 gap-3">
              ...
              <div className="relative w-full sm:w-auto">
                ...
                <input
                  ...
                  className="w-full sm:w-64 pl-8 pr-4 py-1.5 bg-black/20 border border-white/10 rounded-lg text-xs text-white placeholder-zinc-500 outline-none focus:border-violet-500/50 transition-colors"
                />
              </div>
            </div>
                      ...
                      {electionPositions.map((pos) => {
                        ...
                        return (
                          <div key={pos.id} className="space-y-2">
                            <h5 className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider pl-1 border-l-2 border-violet-500/40">
                              {pos.name}
                            </h5>
                              ...
                              <AnimatePresence mode="popLayout">
                                {positionCandidates.map((cand) => (
                                  <motion.div
                                    ...
                                    className="flex items-center justify-between bg-white/2 border border-white/5 p-4 rounded-xl transition-all shadow-sm"
                                  >
                                    <div
                                    ...
                                    >
                                      ...
                                      <p className="text-xs text-zinc-400 mt-1 line-clamp-2 italic leading-relaxed max-w-md group-hover:text-zinc-300 transition-colors">
                                        "{cand.manifesto}"
                                      </p>
                                    </div>
                                    <motion.button
                                      ...
                                      className="p-2 text-zinc-400 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all cursor-pointer shrink-0 ml-4"
                                    >
                                      <Trash2 size={14} />
                                    </motion.button>
                                  </motion.div>
